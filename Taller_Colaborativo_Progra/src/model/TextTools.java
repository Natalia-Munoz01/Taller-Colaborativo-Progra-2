...
public class TextTools {
	 ...
	 * @return palabra con letra inicial en mayuscula siempre y cuando tengan mas de
	 *         un caracter
	 */
	...
	/**
	 * @param character caracter a verificar su existencia dentro de la cadena de
	 *                  inteseccion
	 * @param words     cadena formada apartir de la interseccion entre cadenas
	 * @return verdadero si el caracter no esta o falso si se encuentra
	 */
	public static boolean identifyRepeatingData(String character, String words) {
		int indicator = 0;
		for (int i = 0; i < words.length(); i++) {
			if (words.substring(i, i + 1).equalsIgnoreCase(character)) {
				indicator++;
			}
		}
		if (indicator > 0)
